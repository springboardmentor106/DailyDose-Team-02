...
const Register = () => {
  ...
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;      // This ids for Regex pattern for email validation
...
// we are using Regex pattern for password validation
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.password) {
      ...
      newValidationMessages.password = 'Password must contain at least 8 characters, including one uppercase letter, one lowercase letter, and one number.';
    }
    if (!formData.gender) {
      ...
      <div className="inner">
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            <input type="text" placeholder="First Name" className="form-control" name="firstName" value={formData.firstName} onChange={handleChange} />
            {validationMessages.firstName && <p className="validation-message">{validationMessages.firstName}</p>}
            <input type="text" placeholder="Last Name" className="form-control" name="lastName" value={formData.lastName} onChange={handleChange} />
            {validationMessages.lastName && <p className="validation-message">{validationMessages.lastName}</p>}
          </div>
          <div className="form-wrapper">
            <input type="text" placeholder="Email Address" className="form-control" name="email" value={formData.email} onChange={handleChange} />
          ...
          <div className="form-wrapper">
            ...
            {validationMessages.gender && <p className="validation-message">{validationMessages.gender}</p>}
          ...
          <div className="form-wrapper">
            ...
            {validationMessages.password && <p className="validation-message">{validationMessages.password}</p>}
          ...
          <div id="login-btns">
            ...
            <a href="f">
              <div className="logo" style={{ width: "32px", height: "32px", borderRadius: "50%" }}>
                <img
