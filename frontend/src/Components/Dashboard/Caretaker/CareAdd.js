...
const CareAdd = ({ unAssignedUserDetails, setRefresh }) => {
  ...
  const assignUserToCaretaker = async (userId) => {
    try {
      ...
      const response = await fetch(Constants.BASE_URL + '/api/caretaker/assign-user', {
        method: "POST",
        headers: {
          ...
          <div className="accordion-item" key={index}>
            <h2 className="accordion-header">
              ...
              >
                ...
                <div>
                  <button type="button" className="btn btn-light btn-sm" id="getAssign" style={{ marginLeft: "10px" }}>
                    Get Assigned
                  </button>
                </div>
                <br />
                <div>
                  <button type="button" className="btn btn-light btn-sm" id="getAssign">
                    Show More
                  </button>
                </div>
              </button>
                  ...
                  <div className="home-row-one">
                    <div className="card">
                      <div className="card-body">
                        ...
                        <div>{unassign.goals.length ? <CircularProgressBar value={unassign.progress.complete || 0} /> : <div>No goals yet. rogress will be updated when you complete your goals.</div>}</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="modal-dialog">
                    <div className="modal-content">
                      <div className="modal-header">
                        <h1 className="modal-title fs-5" id="exampleModalLabel">Caretaker Responsibility</h1>
                      ...
                      <div className="modal-body">
                        ...
                        <br />
                        <p>If you have any questions about your responsibilities as a caretaker, Please contact the User. Thank you for your willingness to be a caretaker!</p>
                      </div>
                      <div className="modal-footer">
                        <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" className="btn btn-primary" onClick={() => assignUserToCaretaker(unassign.uuid)}>Confirm</button>
                      </div>
                    </div>
                  </div>
                </div>
